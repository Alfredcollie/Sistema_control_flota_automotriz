...
class OrdenesCompraClienteApp:
    ...
    def inicializar_bd(self):
        ...
        try:
            ...
            # 🚀 FORZAMOS LA CREACIÓN DE TODAS LAS COLUMNAS POR SI LA TABLA ES VIEJA
            try: cursor.execute("ALTER TABLE ordenes_compra_clientes ADD COLUMN cliente VARCHAR(255) DEFAULT ''"); conn.commit()
            except: conn.rollback()
            try: cursor.execute("ALTER TABLE ordenes_compra_clientes ADD COLUMN monto_total NUMERIC DEFAULT 0"); conn.commit()
            except: conn.rollback()
            try: cursor.execute("ALTER TABLE ordenes_compra_clientes ADD COLUMN descripcion TEXT DEFAULT ''"); conn.commit()
            except: conn.rollback()
            try: cursor.execute("ALTER TABLE ordenes_compra_clientes ADD COLUMN subtotal NUMERIC DEFAULT 0"); conn.commit()
            except: conn.rollback()
            try: cursor.execute("ALTER TABLE ordenes_compra_clientes ADD COLUMN igv NUMERIC DEFAULT 0"); conn.commit()
            except: conn.rollback()
    ...
    def crear_interfaz(self):
        ...
        ctk.CTkLabel(f_form, text="1. N° Orden de Compra:", font=("Arial", 11, "bold")).pack(anchor="w", padx=15)
        ...
        self.ent_busc_cli.bind("<KeyRelease>", lambda e: self.cargar_tabla())

        style = ttk.Style()
        style.configure("Treeview", rowheight=28, font=("Arial", 10))
        style.configure("Treeview.Heading", font=("Arial", 10, "bold"))

        columnas = ("id", "oc", "fecha", "cotizacion", "cliente", "desc", "subtotal", "igv", "monto", "arch")
        self.tbl_cli = ttk.Treeview(f_derecha, columns=columnas, show="headings")
        self.tbl_cli.heading("oc", text="N° OC")
        ...
        ctk.CTkButton(f_btn_del, text="❌ Eliminar Registro Seleccionado", font=("Arial", 12, "bold"), fg_color="#e74c3c", hover_color="#c0392b", command=self.eliminar_oc).pack(side="right")
    ...
    def escanear_pdf(self):
        ...
        try:
            ...
            m_oc = re.search(r"(?:ORDEN DE COMPRA|ORDEN DE SERVICIO|O/C|OC|PO|PURCHASE ORDER)[\sN°:#]+([A-Z0-9\-]+)", texto, re.IGNORECASE)
            if m_oc:
            ...
            if m_sub: sub_val = float(m_sub.group(1).replace(",", ""))
            if m_igv: igv_val = float(m_igv.group(1).replace(",", ""))
            if m_tot: tot_val = float(m_tot.group(1).replace(",", ""))
            ...
            if sub_val > 0:
                self.ent_subtotal.delete(0, tk.END); self.ent_subtotal.insert(0, f"{sub_val:.2f}")
                self.ent_igv.delete(0, tk.END); self.ent_igv.insert(0, f"{igv_val:.2f}")
                self.ent_monto_cli.delete(0, tk.END); self.ent_monto_cli.insert(0, f"{tot_val:.2f}")
            ...
            if filas:
                ...
                if idx_cod != -1:
                    for r in filas:
                        ...
                        if cod_val in ocs_asignadas: continue 
    ...
    def guardar_oc(self):
        ...
        if not oc:
            return messagebox.showwarning("Atención", "Debe ingresar el N° de Orden de Compra.")
        ...
        except: return messagebox.showerror("Error", "Los valores financieros deben ser números válidos.")
        ...
        try:
            ...
            conn.commit()
            registrar_auditoria(self.usuario_activo, "Órdenes de Compra Clientes", f"Recibió y archivó OC {oc} vinculada a {cot_codigo}")
    ...
    def cargar_tabla(self):
        for f in self.tbl_cli.get_children(): self.tbl_cli.delete(f)
        filtro = self.ent_busc_cli.get().lower()
        
        conn = conectar_db()
        if not conn: return
        try:
            c = conn.cursor()
            c.execute("SELECT id, numero_oc, fecha, cotizacion_asociada, cliente, descripcion, subtotal, igv, monto_total, archivo_ruta FROM ordenes_compra_clientes ORDER BY id DESC")
            for r in c.fetchall():
                if filtro and filtro not in str(r[1]).lower() and filtro not in str(r[4]).lower() and filtro not in str(r[3]).lower() and filtro not in str(r[5]).lower():
                    continue
    ...
    def eliminar_oc(self):
        sel = self.tbl_cli.selection()
        if not sel: return messagebox.showwarning("Aviso", "Seleccione un registro para eliminar.")
        ...
        if messagebox.askyesno("Confirmar", f"¿Eliminar orden {num_oc} permanentemente?\n\nAl hacer esto, la cotización asociada volverá a aparecer como disponible en el desplegable."):
            ...
            if conn:
                try:
                    ...
                    conn.commit()
                    registrar_auditoria(self.usuario_activo, "Órdenes de Compra Clientes", f"Eliminó OC Recibida {num_oc}")
                except: pass
                finally: conn.close()
