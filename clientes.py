...
def inicializar_db_clientes():
    ...
    try:
        ...
        # 🚀 AGREGAMOS LA COLUMNA DE RAZÓN COMERCIAL SI NO EXISTE
        try:
...
class SistemaClientes:
    ...
    def crear_tab_buscar(self):
        ...
        self.ent_buscar.bind("<KeyRelease>", lambda e: self.cargar_clientes_tabla())
        self.ent_buscar.bind("<Return>", lambda e: self.cargar_clientes_tabla())
        ...
        ToolTip(self.btn_editar, "Modifica los datos del cliente seleccionado.")
        ...
        ToolTip(self.btn_eliminar_c, "Borra permanentemente el registro seleccionado.")

        frame_tabla = ctk.CTkFrame(self.tab_buscar, corner_radius=10)
        frame_tabla.pack(fill="both", expand=True, padx=10, pady=5)
        
        # 🚀 AÑADIMOS "num" AL INICIO PARA EL CONTADOR VISUAL
        ...
        self.tabla.column("id", width=0, stretch=tk.NO) # 🚀 OCULTAMOS EL ID REAL
        ...
        # 🚀 CONFIGURAMOS QUÉ COLUMNAS SE VEN
    ...
    def ejecutar_eliminacion_cliente(self):
        ...
        id_cli = valores[1]  # 🚀 EL ID REAL AHORA ESTÁ EN LA POSICIÓN 1
        nombre_cli = valores[3] # 🚀 LA RAZÓN SOCIAL ESTÁ EN LA POSICIÓN 3
    ...
    def cargar_clientes_tabla(self):
        ...
        try:
            cursor = conn.cursor()
            if texto == "":
                cursor.execute("SELECT id, ruc, nombre_empresa, COALESCE(razon_comercial, ''), persona_contacto, telefono, correo FROM clientes ORDER BY nombre_empresa ASC")
            else:
                ...
                val = f"%{texto}%"
                cursor.execute(query, (val, val, val, val, val, val, val, val))
            
            rows = cursor.fetchall()
            contador_visual = 1
            for row in rows: 
                # 🚀 INSERTAMOS EL CONTADOR AL INICIO DE LOS VALORES
    ...
    def abrir_ventana_editar(self):
        ...
        id_cli = items[1] # 🚀 EL ID REAL AHORA ESTÁ EN LA POSICIÓN 1
        ...
        if val_limite > 0: ent_e_limite.insert(0, str(val_limite))
